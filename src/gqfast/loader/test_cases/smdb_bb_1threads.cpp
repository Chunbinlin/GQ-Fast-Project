#ifndef smdb_bb_1threads_
#define smdb_bb_1threads_
...
extern inline void smdb_bb_1threads_concept_semtype1_col0_decode_BB(unsigned char* concept_semtype1_col0_ptr, uint32_t concept_semtype1_col0_bytes, uint32_t & concept_semtype1_fragment_size) __attribute__((always_inline));

void* pthread_smdb_bb_1threads_worker(void* arguments);

extern inline void smdb_bb_1threads_predication1_col0_decode_BB_threaded(int thread_id, unsigned char* predication1_col0_ptr, uint32_t predication1_col0_bytes, uint32_t & predication1_fragment_size) __attribute__((always_inline));

extern inline void smdb_bb_1threads_sentence1_col0_decode_BB_threaded(int thread_id, unsigned char* sentence1_col0_ptr, uint32_t sentence1_col0_bytes, uint32_t & sentence1_fragment_size) __attribute__((always_inline));

extern inline void smdb_bb_1threads_predication2_col0_decode_BB_threaded(int thread_id, unsigned char* predication2_col0_ptr, uint32_t predication2_col0_bytes, uint32_t & predication2_fragment_size) __attribute__((always_inline));

extern inline void smdb_bb_1threads_concept_semtype2_col0_decode_BB_threaded(int thread_id, unsigned char* concept_semtype2_col0_ptr, uint32_t concept_semtype2_col0_bytes, uint32_t & concept_semtype2_fragment_size) __attribute__((always_inline));

extern inline void smdb_bb_1threads_concept2_col0_decode_BB_threaded(int thread_id, unsigned char* concept2_col0_ptr, uint32_t concept2_col0_bytes, uint32_t & concept2_fragment_size) __attribute__((always_inline));

void smdb_bb_1threads_concept_semtype1_col0_decode_BB(unsigned char* concept_semtype1_col0_ptr, uint32_t concept_semtype1_col0_bytes, uint32_t & concept_semtype1_fragment_size) {
...
void* pthread_smdb_bb_1threads_worker(void* arguments) {
	...
	for (; concept_semtype1_it < concept_semtype1_fragment_size; concept_semtype1_it++) {
		...
		if(predication1_col0_bytes) {
			...
			smdb_bb_1threads_predication1_col0_decode_BB_threaded(thread_id, predication1_col0_ptr, predication1_col0_bytes, predication1_fragment_size);
			...
			for (uint32_t predication1_it = 0; predication1_it < predication1_fragment_size; predication1_it++) {
				...
				if(sentence1_col0_bytes) {
					...
					smdb_bb_1threads_sentence1_col0_decode_BB_threaded(thread_id, sentence1_col0_ptr, sentence1_col0_bytes, sentence1_fragment_size);
					...
					for (uint32_t sentence1_it = 0; sentence1_it < sentence1_fragment_size; sentence1_it++) {
						...
						if (!(sentence1_bool_array[index2_col0_buffer[thread_id][0][sentence1_it]])) {
							...
							if(predication2_col0_bytes) {
								...
								smdb_bb_1threads_predication2_col0_decode_BB_threaded(thread_id, predication2_col0_ptr, predication2_col0_bytes, predication2_fragment_size);
								...
								for (uint32_t predication2_it = 0; predication2_it < predication2_fragment_size; predication2_it++) {
									...
									if(concept_semtype2_col0_bytes) {
										...
										smdb_bb_1threads_concept_semtype2_col0_decode_BB_threaded(thread_id, concept_semtype2_col0_ptr, concept_semtype2_col0_bytes, concept_semtype2_fragment_size);
										...
										for (uint32_t concept_semtype2_it = 0; concept_semtype2_it < concept_semtype2_fragment_size; concept_semtype2_it++) {
											...
											if(concept2_col0_bytes) {
												...
												smdb_bb_1threads_concept2_col0_decode_BB_threaded(thread_id, concept2_col0_ptr, concept2_col0_bytes, concept2_fragment_size);
...
void smdb_bb_1threads_predication1_col0_decode_BB_threaded(int thread_id, unsigned char* predication1_col0_ptr, uint32_t predication1_col0_bytes, uint32_t & predication1_fragment_size) {

	index1_col0_buffer[thread_id][0][0] = 0;
	...
	do { 
		...
		next_seven_bits = next_seven_bits << shiftbits;
		index1_col0_buffer[thread_id][0][0] |= next_seven_bits;
		shiftbits += 7;
	...
	while (predication1_col0_bytes > 0) {
		...
		index1_col0_buffer[thread_id][0][predication1_fragment_size] = index1_col0_buffer[thread_id][0][predication1_fragment_size-1]+1+result;
		predication1_fragment_size++;
	}
}

void smdb_bb_1threads_sentence1_col0_decode_BB_threaded(int thread_id, unsigned char* sentence1_col0_ptr, uint32_t sentence1_col0_bytes, uint32_t & sentence1_fragment_size) {
	...
	while (sentence1_col0_bytes > 0) {
		...
		index2_col0_buffer[thread_id][0][sentence1_fragment_size] = index2_col0_buffer[thread_id][0][sentence1_fragment_size-1]+1+result;
		sentence1_fragment_size++;
	}
}

void smdb_bb_1threads_predication2_col0_decode_BB_threaded(int thread_id, unsigned char* predication2_col0_ptr, uint32_t predication2_col0_bytes, uint32_t & predication2_fragment_size) {
	...
	while (predication2_col0_bytes > 0) {
		...
		index3_col0_buffer[thread_id][0][predication2_fragment_size] = index3_col0_buffer[thread_id][0][predication2_fragment_size-1]+1+result;
		predication2_fragment_size++;
	}
}

void smdb_bb_1threads_concept_semtype2_col0_decode_BB_threaded(int thread_id, unsigned char* concept_semtype2_col0_ptr, uint32_t concept_semtype2_col0_bytes, uint32_t & concept_semtype2_fragment_size) {

	index4_col0_buffer[thread_id][0][0] = 0;
	...
	do { 
		...
		next_seven_bits = next_seven_bits << shiftbits;
		index4_col0_buffer[thread_id][0][0] |= next_seven_bits;
		shiftbits += 7;
	...
	while (concept_semtype2_col0_bytes > 0) {
		...
		index4_col0_buffer[thread_id][0][concept_semtype2_fragment_size] = index4_col0_buffer[thread_id][0][concept_semtype2_fragment_size-1]+1+result;
		concept_semtype2_fragment_size++;
	}
}

void smdb_bb_1threads_concept2_col0_decode_BB_threaded(int thread_id, unsigned char* concept2_col0_ptr, uint32_t concept2_col0_bytes, uint32_t & concept2_fragment_size) {

	index5_col0_buffer[thread_id][0][0] = 0;
	...
	do { 
		...
		next_seven_bits = next_seven_bits << shiftbits;
		index5_col0_buffer[thread_id][0][0] |= next_seven_bits;
		shiftbits += 7;
	...
	while (concept2_col0_bytes > 0) {
		...
		index5_col0_buffer[thread_id][0][concept2_fragment_size] = index5_col0_buffer[thread_id][0][concept2_fragment_size-1]+1+result;
		concept2_fragment_size++;
	}
}

extern "C" int* smdb_bb_1threads(int** null_checks) {
	...
	for (int concept1_it = 0; concept1_it<1; concept1_it++) {
		...
		if(concept_semtype1_col0_bytes) {
			...
			smdb_bb_1threads_concept_semtype1_col0_decode_BB(concept_semtype1_col0_ptr, concept_semtype1_col0_bytes, concept_semtype1_fragment_size);
			...
			for (int i=0; i<NUM_THREADS; i++) {
				pthread_create(&threads[i], NULL, &pthread_smdb_bb_1threads_worker, (void *) &arguments[i]);
			}
