#ifndef smdb_bca_2threads_
#define smdb_bca_2threads_
...
extern inline void smdb_bca_2threads_concept_semtype1_col0_decode_BCA(unsigned char* concept_semtype1_col0_ptr, uint32_t concept_semtype1_col0_bytes, uint32_t & concept_semtype1_fragment_size) __attribute__((always_inline));

void* pthread_smdb_bca_2threads_worker(void* arguments);

extern inline void smdb_bca_2threads_predication1_col0_decode_BCA_threaded(int thread_id, unsigned char* predication1_col0_ptr, uint32_t predication1_col0_bytes, uint32_t & predication1_fragment_size) __attribute__((always_inline));

extern inline void smdb_bca_2threads_sentence1_col0_decode_BCA_threaded(int thread_id, unsigned char* sentence1_col0_ptr, uint32_t sentence1_col0_bytes, uint32_t & sentence1_fragment_size) __attribute__((always_inline));

extern inline void smdb_bca_2threads_predication2_col0_decode_BCA_threaded(int thread_id, unsigned char* predication2_col0_ptr, uint32_t predication2_col0_bytes, uint32_t & predication2_fragment_size) __attribute__((always_inline));

extern inline void smdb_bca_2threads_concept_semtype2_col0_decode_BCA_threaded(int thread_id, unsigned char* concept_semtype2_col0_ptr, uint32_t concept_semtype2_col0_bytes, uint32_t & concept_semtype2_fragment_size) __attribute__((always_inline));

extern inline void smdb_bca_2threads_concept2_col0_decode_BCA_threaded(int thread_id, unsigned char* concept2_col0_ptr, uint32_t concept2_col0_bytes, uint32_t & concept2_fragment_size) __attribute__((always_inline));

void smdb_bca_2threads_concept_semtype1_col0_decode_BCA(unsigned char* concept_semtype1_col0_ptr, uint32_t concept_semtype1_col0_bytes, uint32_t & concept_semtype1_fragment_size) {
	...
	for (uint32_t i=0; i<concept_semtype1_fragment_size; i++) {
		...
		index0_col0_buffer[0][0][i] = encoded_value + concept_semtype1_col0_offset;
	}
}
...
void* pthread_smdb_bca_2threads_worker(void* arguments) {
	...
	for (; concept_semtype1_it < concept_semtype1_fragment_size; concept_semtype1_it++) {
		...
		if(predication1_col0_bytes) {
			...
			smdb_bca_2threads_predication1_col0_decode_BCA_threaded(thread_id, predication1_col0_ptr, predication1_col0_bytes, predication1_fragment_size);
			...
			for (uint32_t predication1_it = 0; predication1_it < predication1_fragment_size; predication1_it++) {
				...
				if(sentence1_col0_bytes) {
					...
					smdb_bca_2threads_sentence1_col0_decode_BCA_threaded(thread_id, sentence1_col0_ptr, sentence1_col0_bytes, sentence1_fragment_size);
					...
					for (uint32_t sentence1_it = 0; sentence1_it < sentence1_fragment_size; sentence1_it++) {
						...
						if (!(sentence1_bool_array[index2_col0_buffer[thread_id][0][sentence1_it]])) {
							...
							if(predication2_col0_bytes) {
								...
								smdb_bca_2threads_predication2_col0_decode_BCA_threaded(thread_id, predication2_col0_ptr, predication2_col0_bytes, predication2_fragment_size);
								...
								for (uint32_t predication2_it = 0; predication2_it < predication2_fragment_size; predication2_it++) {
									...
									if(concept_semtype2_col0_bytes) {
										...
										smdb_bca_2threads_concept_semtype2_col0_decode_BCA_threaded(thread_id, concept_semtype2_col0_ptr, concept_semtype2_col0_bytes, concept_semtype2_fragment_size);
										...
										for (uint32_t concept_semtype2_it = 0; concept_semtype2_it < concept_semtype2_fragment_size; concept_semtype2_it++) {
											...
											if(concept2_col0_bytes) {
												...
												smdb_bca_2threads_concept2_col0_decode_BCA_threaded(thread_id, concept2_col0_ptr, concept2_col0_bytes, concept2_fragment_size);
...
void smdb_bca_2threads_predication1_col0_decode_BCA_threaded(int thread_id, unsigned char* predication1_col0_ptr, uint32_t predication1_col0_bytes, uint32_t & predication1_fragment_size) {
	...
	for (uint32_t i=0; i<predication1_fragment_size; i++) {
		...
		index1_col0_buffer[thread_id][0][i] = encoded_value + predication1_col0_offset;
	}
}

void smdb_bca_2threads_sentence1_col0_decode_BCA_threaded(int thread_id, unsigned char* sentence1_col0_ptr, uint32_t sentence1_col0_bytes, uint32_t & sentence1_fragment_size) {
	...
	for (uint32_t i=0; i<sentence1_fragment_size; i++) {
		...
		index2_col0_buffer[thread_id][0][i] = encoded_value + sentence1_col0_offset;
	}
}

void smdb_bca_2threads_predication2_col0_decode_BCA_threaded(int thread_id, unsigned char* predication2_col0_ptr, uint32_t predication2_col0_bytes, uint32_t & predication2_fragment_size) {
	...
	for (uint32_t i=0; i<predication2_fragment_size; i++) {
		...
		index3_col0_buffer[thread_id][0][i] = encoded_value + predication2_col0_offset;
	}
}

void smdb_bca_2threads_concept_semtype2_col0_decode_BCA_threaded(int thread_id, unsigned char* concept_semtype2_col0_ptr, uint32_t concept_semtype2_col0_bytes, uint32_t & concept_semtype2_fragment_size) {
	...
	for (uint32_t i=0; i<concept_semtype2_fragment_size; i++) {
		...
		index4_col0_buffer[thread_id][0][i] = encoded_value + concept_semtype2_col0_offset;
	}
}

void smdb_bca_2threads_concept2_col0_decode_BCA_threaded(int thread_id, unsigned char* concept2_col0_ptr, uint32_t concept2_col0_bytes, uint32_t & concept2_fragment_size) {
	...
	for (uint32_t i=0; i<concept2_fragment_size; i++) {
		...
		index5_col0_buffer[thread_id][0][i] = encoded_value + concept2_col0_offset;
	}
}

extern "C" int* smdb_bca_2threads(int** null_checks) {
	...
	for (int concept1_it = 0; concept1_it<1; concept1_it++) {
		...
		if(concept_semtype1_col0_bytes) {
			...
			smdb_bca_2threads_concept_semtype1_col0_decode_BCA(concept_semtype1_col0_ptr, concept_semtype1_col0_bytes, concept_semtype1_fragment_size);
			...
			for (int i=0; i<NUM_THREADS; i++) {
				pthread_create(&threads[i], NULL, &pthread_smdb_bca_2threads_worker, (void *) &arguments[i]);
			}
