#ifndef q1_array_2threads_
#define q1_array_2threads_
...
extern inline void q1_array_2threads_term1_col0_decode_UA(uint32_t* term1_col0_ptr, uint32_t & term1_fragment_size) __attribute__((always_inline));

void* pthread_q1_array_2threads_worker(void* arguments);

extern inline void q1_array_2threads_doc2_col0_decode_UA_threaded(int thread_id, uint32_t* doc2_col0_ptr, uint32_t & doc2_fragment_size) __attribute__((always_inline));

void q1_array_2threads_term1_col0_decode_UA(uint32_t* term1_col0_ptr, uint32_t & term1_fragment_size) {
...
void* pthread_q1_array_2threads_worker(void* arguments) {
	...
	for (; term1_it < term1_fragment_size; term1_it++) {
		...
		if(frag_doc2) {
			...
			doc2_frag_ptr++;
			q1_array_2threads_doc2_col0_decode_UA_threaded(thread_id, doc2_frag_ptr, doc2_frag_size);
...
void q1_array_2threads_doc2_col0_decode_UA_threaded(int thread_id, uint32_t* doc2_col0_ptr, uint32_t & doc2_fragment_size) {
...
extern "C" int* q1_array_2threads(int** null_checks, int doc1) {
	...
	for (int doc1_it = 0; doc1_it<1; doc1_it++) {
		...
		if(frag_term1) {
			...
			q1_array_2threads_term1_col0_decode_UA(frag_term1_ptr, term1_frag_size);
			...
			for (int i=0; i<NUM_THREADS; i++) {
				pthread_create(&threads[i], NULL, &pthread_q1_array_2threads_worker, (void *) &arguments[i]);
			}
