...
void pubmed_create_indices(int my_encoding, bool tag_flag)
{

    if (tag_flag)
    {
        Encodings* encoding1 = new Encodings("Type", my_encoding);
        idx[0] = buildIndex<TValue, TIndexMap>("./nodes_tag.csv", encoding1, 0);
        init_buffer(0);

        Encodings* encoding2 = new Encodings("Dest", my_encoding);
        idx[1] = buildIndex<TValue, TIndexMap>("./edges_tag.csv", encoding2, 1);
        init_buffer(1);
    ...
    {
        Encodings* encoding1 = new Encodings("Type", my_encoding);
        idx[0] = buildIndex<TValue, TIndexMap>("./nodes_mesh.csv", encoding1, 0);
        init_buffer(0);

        Encodings* encoding2 = new Encodings("Dest", my_encoding);
        idx[1] = buildIndex<TValue, TIndexMap>("./edges_mesh.csv", encoding2, 1);
        init_buffer(1);
    }
...
int load(int database, int compression)
{
    ...
    {
        if (compression >= 0 && compression <= NUM_ENCODING_TYPES_SUPPORTED)
        {
            pubmed_create_indices<TValue, TIndexMap>(compression, false);
            return 1;
        }
    }
    else if (database == PUBMED_MESH_PLUS_SUPP)
    {
        if (compression >= 0 && compression <= NUM_ENCODING_TYPES_SUPPORTED)
        {
            pubmed_create_indices<TValue, TIndexMap>(compression, true);
            return 1;
        }
    }
