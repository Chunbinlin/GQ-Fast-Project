#ifndef gqfast_run_query_
#define gqfast_run_query_
...
extern pthread_spinlock_t** spin_locks;
...
void handle_input(string func_name, int r_pos)
{
    ...
    void* handle = dlopen(func_name.c_str(), RTLD_NOW);
    if (!handle)
    {
    ...
    cout << "Query " << func_name << " processed in " << time_span.count() << " seconds.\n\n";
