#ifndef GqFastIndex_hpp_
#define GqFastIndex_hpp_
...
public:
    ...
    // Huffman related
    uint32_t**  huffman_tree_array;
    bool** huffman_terminator_array;




    GqFastIndex(uint64_t ds, int nfd, unsigned int* fdl, int* htas, bool flag) : domain_size(ds), num_fragment_data(nfd),
        fragment_data_length(fdl), huffman_tree_array_size(htas), load_flag(flag)
    {
        ...
        huffman_tree_array = new uint32_t*[num_fragment_data];
    ...
    // Constructor
    GqFastIndex(int ds, int nfd, unsigned int* fdl, int* fet) : domain_size(ds),
    ...
    void set_huffman(uint32_t** hta, bool** hterma, int* htas)
    {
...
inline void save_construct_data(
    Archive & ar, const GqFastIndex<TIndexMap> * t, const unsigned int file_version
)
{
...
inline void load_construct_data(
    Archive & ar, GqFastIndex<TIndexMap> * t, const unsigned int file_version
)
{
    ...
    ::new(t)GqFastIndex<TIndexMap>(domain_size, num_fragment_data, fragment_data_length, huffman_tree_array_size, load_flag);
}
